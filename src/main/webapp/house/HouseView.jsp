...
<div class="main_text1">
	<h1>${views.houseName}</h1>
	<div class="contents1">${views.scontent}</div>
	<div class="service">
		<div class="house_photo">
			<img src="/together/house/img/${views.ofile}" />
		</div>
		<div class="contents2">
...
<div class ="styleForm">
	<c:choose>
		<c:when test="${sessionScope.principal.id == views.id}">
			<form method="post"
				action="<%= request.getContextPath() %>/house?cmd=edit&num=${views.num}">
				<button class="btn btn-primary pull-right">수정하기</button>
			</form>
			<form method="post"
				action="<%= request.getContextPath() %>/house?cmd=delete&num=${views.num}">
				<button class="btn btn-primary pull-right" >삭제하기</button>
			</form>
		</c:when>
		<c:otherwise>
			<button class="btn btn-blue" id="liketo"></button>
		</c:otherwise>
	</c:choose>
</div>
<!-- 댓글 박스 -->
