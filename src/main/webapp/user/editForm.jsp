...
function passCheck(){
	...
	<form action="<%=request.getContextPath()%>/user?cmd=edit" method="post" enctype="multipart/form-data" onsubmit="return passCheck()">
		...
		<div>
	 		비밀번호 *
	 		<input type="password" name="userPass1" id="userPass1" value="${sessionScope.principal.pw1} " required>
		</div>
		<div>
	 		비밀번호 확인 *
		...
		<div>
	 		강아지 사진 
	 		<input type="file" name="photo" id="photo" src="/together/user/profile_img/${sessionScope.principal.ofile}" accept = "image/*">
		</div>
		<!-- 등록할 강아지 사진 미리보기 div -->
		<c:choose>
